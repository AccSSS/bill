...
let delBillLog = (id) => {
    let _sql = `delete from bill_log where id="${id}";`
    return query(_sql)
}

let getBillLogInfo = (id) => {
    let _sql = `select * from bill_log where id="${id}";`
    return query(_sql)
}

let updateBillLogInfo = (id,data) => {
    let _sql = `update bill_log set money = "${data.money}", time = "${data.time}", descr = "${data.descr}", type = "${data.type}" where id="${id}";`
    return query(_sql)
}
